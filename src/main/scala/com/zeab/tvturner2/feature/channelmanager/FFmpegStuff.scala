...
trait FFmpegStuff {
  ...
  def split(file: File): Int ={
    ...
    Process(s"${AppConf.ffmpegPath} -i ${file.getAbsoluteFile} -map 0 -codec copy -f segment -segment_time 00:30 ${new File(AppConf.tempPath).getAbsolutePath}/${file.getName}%03d.ts").!
